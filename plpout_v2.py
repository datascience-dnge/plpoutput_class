...
class plpout:
	...
	def plpbar(self):
		...
		if self.horario: 
			plpbars=plpbar_df.join(indhor_df,["Bloque"])\
						     .selectExpr("cmgbar","hora","Hidro","fecha","barnom")\
	...
	def plpcen(self):
		...
		if self.horario:
			plpcens=plpcen_df.join(indhor_df,["Bloque"])\
						     .selectExpr("cenpgen as fisico","hora","Hidro","cennom","ceninyp as valorizado","fecha","barnom","cencvar")\
	...
	def plp_bar_cen(self):
		...
		if self.horario:
			plpbar_df=self.plpbar()
			plpcen_df=self.plpcen() 
